...
void cashier_scan_items(Basket *basket, StockList *stock) {
  ...
  do {
    ...
    if (strlen(item_raw_barecode) == 0) {
      if (user_yes_no(
              "Voulez-vous continuer a ajouter des articles au panier ?",
              YES) == NO) {
        exited = true;
      }
    } else {
      ...
      int total_asked_quantity =
          ((item_in_basket ? item_in_basket->quantity : 0) + item_quantity);

      if (total_asked_quantity > item->quantity) {
        log_error("Stock insufisants (%d en stock mais %d demander)",
                  item->quantity, total_asked_quantity);
        continue;
      }
